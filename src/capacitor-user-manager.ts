...
export abstract class CapacitorUserManager extends UserManager {
  ...
  static async create(
    ...
    let manager: CapacitorUserManager;
    if (resolved.platform === 'web') {
      const { WebCapacitorUserManager } = await import('./web-capacitor-user-manager.js');
      manager = new WebCapacitorUserManager(resolved);
    } else {
      const { NativeCapacitorUserManager } = await import('./native-capacitor-user-manager.js');
      manager = await NativeCapacitorUserManager.fromConfiguration(resolved);
    }
