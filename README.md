...
> [!WARNING]
> This package is not ready to publish. `oidc-client-ts` 3.5.0 does not validate the required ID-token `iss`, `aud`, and `exp` claims. See [dependency gates](docs/DEPENDENCY_GATES.md).
...
```sh
npm install @jojopirker/capacitor-oidc oidc-client-ts @capacitor/app
npx cap sync
```
...
```ts
import { CapacitorUserManager } from '@jojopirker/capacitor-oidc';
...
```swift
let vault = TokenVault(accessGroup: "group.com.example.app")
let session = try vault.loadSession(namespace: "primary")
```

Both targets must carry the same Keychain Sharing entitlement. The default accessibility is `AfterFirstUnlockThisDeviceOnly`; Keychain synchronization and biometric gating are disabled.
